...
// sur l'élément qui déclenche l'event
<button onClick={() => handleClick(id)}>Cliquer</button>;
```

Pour le typage, si c'est un event que l'on viens récupérer dans la fonction handler, on peut trouver le type à utiliser en laissant le curseur de la sourir l'event.
Par exemple, en survolant le `e`, l'IDE fournis le type de l'argument.
...
On peut ensuite venir typage l'argument du handler
...
React fournis également des types prédéfinis

| Event       | Type                  |
| ----------- | --------------------- |
| ` onClick`  | `React.MouveEvent`    |
...
On peut également venir typer le handler
...
Si le handler doit prendre des valeur précise, on peut venir utiliser des union de type

```tsx
type FormType = "age" | "name" | "email";
...
// on passe l'event
<Button onClick={() => handleSelectForm("age")}>Par âge</Button>;
```
...
Prendre la v3 pour compatibilité avec React Hook Form
